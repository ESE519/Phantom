...
void change_mode()
{
    ...
    if(mode_value==2) mode_value=1;
    while(1)
    {
       pc.printf("change mode :%d\r\n",mode_value );
       if(t.read()>5)
       {
          t.reset();
          break;
        ...
        else
        {
           if(mode_value==1)
           {
               if(adc-adc_temp>80)
               {
                  ...
                  {
                     pc.printf("FORWARD\r\n");
                      msg.mode_send = FORWARD;
                  }
                  else if(angle_x>25 && angle_x<50)
                  {
                     pc.printf("LEFT_TURN_30\r\n");
                      msg.mode_send = LEFT_TURN_30;
                  }
                  else if(angle_x>50 && angle_x<75)
                  {
                      pc.printf("LEFT_TURN_60\r\n");
                      msg.mode_send = LEFT_TURN_60;
                  }
                  else if(angle_x>75 && angle_x<90)
                  {
                      pc.printf("LEFT_CLOCK\r\n");
                      msg.mode_send = LEFT_CLOCK;
                  }
                  else if(angle_x<-25 && angle_x>-50)
                  {
                     pc.printf("RIGHT_TURN_30\r\n");
                      msg.mode_send = RIGHT_TURN_30;
                  }
                  else if(angle_x<-50 && angle_x>-75)
                  {
                     pc.printf("RIGHT_TURN_60\r\n");
                      msg.mode_send = RIGHT_TURN_60;
                  }
                  else if(angle_x<-75 && angle_x>-90)
                  {
                     pc.printf("RIGHT_CLOCK\r\n");
                      msg.mode_send = RIGHT_CLOCK;
                  }
                  else
                  {
                     pc.printf("IDLE\r\n");
               ...
               else if(adc_temp-adc>0)
               {
                  if(abs(angle_x)<60)
                  {
                     pc.printf("BACKWARD\r\n");
                      msg.mode_send = BACKWARD;
                  }
                  else if(angle_x>75 && angle_x<90 )
                  {
                     pc.printf("LEFTWARD\r\n");
                      msg.mode_send = LEFTWARD;
                  }
                  else if(angle_x<-75 && angle_x>-90)
                  {
                     pc.printf("RIGHTWARD\r\n");
                      msg.mode_send = RIGHTWARD;
                  }
                  else
                  {
                     pc.printf("IDLE\r\n");
